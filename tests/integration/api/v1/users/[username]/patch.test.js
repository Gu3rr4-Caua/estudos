  ...
  describe("Anonymous user", () => {
    test("With nonexistent 'username'", async () => {
      ...
      expect(responseBody).toEqual({
        name: "NotFoundError",
        message: "O usarnem informado nao foi encontrado no sistema.",
    ...
    test("With unique 'email'", async () => {
      const createUserUnique = await orchestrator.createUser({
        email: "uniqueEmail1@curso.dev",
      });

      const response = await fetch(
        `http://localhost:3000/api/v1/users/${createUserUnique.username}`,
        {
          method: "PATCH",
          headers: {
            "content-Type": "application/json",
